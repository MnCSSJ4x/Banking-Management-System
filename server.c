...
//Recieve a A type of operation (Normal , Joint, Admin),Query q ,Check the business logic in backend , 
...
bool checkNormalUser(normal c){
	...
	if(!strcmp(t.password,c.password) && !strcmp(t.details,"ACTIVE"))	res=true;
	else res=false;
...
bool checkAdmin(admin c){
	...
	if(!strcmp(t.password,c.password))	res=true;
	else res=false;
...
float getBalance(int account_type,int ID){
    ...
    case 1:
		...
		if(!strcmp(temp.details,"ACTIVE"))	result=temp.balance;
		else result=0;
	...
	case 2:
		...
		lseek(fd,(i)*sizeof(joint),SEEK_SET); 
		read(fd,&j,sizeof(j));
		if(!strcmp(j.details,"ACTIVE"))	result=j.balance;
		else result=0;
...
bool alterPassword(int account_type, int ID, char newpass[10]){
	int i=ID-1000;
	int fd;
	int fl1;
	bool result ;
...
void server(int nsd){
	...
	while(1){
		...
		if(option==1){
			normal currUser1;
			accType=1;
			msgLength=read(nsd,&currUser1,sizeof(normal));
			printf("Username : %d\n",currUser1.id);
			printf("Password : %s\n",currUser1.password);
		...
		else if(option==2){joint currUser2;
			accType=2;
			msgLength=read(nsd,&currUser2,sizeof(joint));
	...
	while(1){
		...
		if(option==1 || option==2){
			if(q.query_num==1){
				...
				if(option==1){
					normal user1=getNormalUser(currUserID);
					write(nsd,&user1,sizeof(normal));
				}
				else if(option==2){
					joint user2=getJointUser(currUserID);
					write(nsd,&user2,sizeof(joint));
				}
			}
			else if(q.query_num==6)	break;
		}
		else if(option==3){
			...
			if(q.query_num==1){
				...
				if(type==1){
					normal newUser1;
					read(nsd,&newUser1,sizeof(normal));
					result=addNormalUser(newUser1);
					write(nsd,&result,sizeof(result));
				}
				else if(type==2){
					joint newUser2;
					read(nsd,&newUser2,sizeof(joint));
				...
				if(type==1){
					normal modUser1;
					read(nsd,&modUser1,sizeof(normal));
					result=modifyNormalUser(modUser1);
					write(nsd,&result,sizeof(result));
				}
				else if(type==2){
					joint modUser2;
					read(nsd,&modUser2,sizeof(joint));
				...
				if(type==1){
					...
					write(nsd,&searchUser1,sizeof(normal));
				}
				else if(type==2){
					joint searchUser2;
					int userID2;
					...
					write(nsd,&searchUser2,sizeof(joint));
				}
			}
			else if(q.query_num==5)	break;
		}
	}
	close(nsd);
